...
Item {
    ...
    function begin(entry, newWindow) {
        pendingApp = {
            id: entry.id, name: entry.name, startupClass: entry.startupClass,
        ...
        previous = Hyprland.toplevels.values.map(function(t) { return t.lastIpcObject.address; })
        started = Date.now()
        candidate = ""
        poll.restart()
    }
    function cancel() { poll.stop(); pendingApp = null }
    ...
    Timer {
        ...
        repeat: true
        onTriggered: {
            if (!root.pendingApp || Date.now() - root.started > 15000) { root.cancel(); return }
            Hyprland.refreshToplevels()
            var active = Hyprland.activeToplevel
            var windows = Hyprland.toplevels.values.map(function(t) { return t.lastIpcObject; })
            var target = Focus.choose(windows, root.pendingApp, root.previous,
                active ? active.lastIpcObject.address : "")
            if (!target) { root.candidate = ""; return }
            ...
            if (Date.now() - root.candidateSince < 350) return
            var point = Focus.center(target)
            if (!point || !/^0x[0-9a-f]+$/i.test(target.address)) return
            root.cancel()
